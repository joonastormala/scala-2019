...
class GameState(val missesAllowed: Int, val previousGuesses: String, val visibleWord: String, val possibleSolutions: Vector[String])  {
  ...
  def isWon = numberOfSolutions == 1 && possibleSolutions(0) == visibleWord
  ...
  private def reveal(patternToReveal: String) = {
    visibleWord.indices.map(n => if(patternToReveal(n) == visibleWord(n) || visibleWord(n) != '_') visibleWord(n) else patternToReveal(n)).mkString("")
  }
  ...
  def guessLetter(guess: Char) = {
    ...
    val solutionByGroup = this.possibleSolutions.groupBy(_.map(x => if(x != actualGuess) { '_' } else { actualGuess}))
    val solutionPair = solutionByGroup.maxBy(_._2.size)
    val solution = solutionPair._2
    val howMany = this.previousGuesses.length + 1
    var oneMoreRevealed = visibleWord
    var misses = missesAllowed - 1
    if(solutionPair._1.count(_ == actualGuess) != 0) {
      oneMoreRevealed = reveal(solutionPair._1)
      misses += 1
    }
