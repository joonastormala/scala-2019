...
object SkyFiles {
  ...
  def parseStarInfo(starAsString: String): Star = {
    val splitInfo = starAsString.split(";")
    val xCord = splitInfo(0).toDouble
    val yCord = splitInfo(1).toDouble
    val starID = splitInfo(3).toInt
    val starMag = splitInfo(4).toDouble
    
    val nameOfStar: Option[String] = splitInfo.lift(6)
    
    val thisStar = new StarCoords(xCord, yCord)
    
    new Star(starID, thisStar, starMag, nameOfStar)
  }
