...
class SnakeGame(initialPos: GridPos, initialHeading: CompassDir) {
  ...
  def isOver = {
    ...
    val collideWithSelf = segments.tail.contains(head)
    collidedWithWall || collideWithSelf
  ...
  def advance() = {
    ...
    if(this.segments.head.neighbor(snakeHeading) == nextFood) {
      nextFood = randomLocationOnGrid()
      this.segments = this.segments.head.neighbor(snakeHeading) +: this.segments      
    } 
    else {
      val temp = segments.take(segments.size - 1)
      segments = segments.head.neighbor(snakeHeading) +: temp
    }
