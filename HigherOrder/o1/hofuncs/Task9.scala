...
object Task9 extends App {
  ...
  def positiveCount(numberOfPositives: Int, testIf: Int) = {
    var testnumber = numberOfPositives
    if (testIf > 0) testnumber += 1 else testnumber
    testnumber
  }

  def productOfNonZeros(firstNumber: Int, secondNumber: Int) = {
    var testnumber2 = firstNumber
    if (secondNumber != 0) testnumber2 *= secondNumber else testnumber2
    testnumber2
  }
