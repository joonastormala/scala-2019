...
object Task1 extends App {
  ...
  def toColorimetricGray(color: Color) = {
    val realLuminance = (0.2126*color.red + 0.7152*color.green + 0.0722*color.blue)
    Color(realLuminance, realLuminance, realLuminance)
  }
  ...
  val realGrayPic = originalPic.transformColors(toColorimetricGray)
  originalPic.leftOf(grayPic).leftOf(realGrayPic).show()
