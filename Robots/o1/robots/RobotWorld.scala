...
class RobotWorld(floorWidth: Int, floorHeight: Int) extends Grid[Square](floorWidth + 2, floorHeight + 2) {
  ...
  private def initialSquare(x: Int, y: Int) = {
    if(x < 1 || x == floorWidth + 1 || y < 1 || y == floorHeight + 1) Wall else new Floor
  }
  ...
  def addRobot(initialLocation: GridPos, initialFacing: CompassDir): RobotBody = {
    val newBot = new RobotBody(this, initialLocation, initialFacing)
    this.robots = this.robots :+ newBot
    apply(initialLocation).addRobot(newBot)
    newBot
  ...
  def nextRobot: Option[RobotBody] = robots.lift(turn)
  ...
  def advanceTurn() = {
    nextRobot.map(_.takeTurn())
    if (turn < robots.size - 1) turn += 1 else turn = 0
  }
  ...
  /** Causes all the robots in the world to take a turn, starting with the one whose turn it is next.
    ...
    * @see [[RobotBody.takeTurn]] */
  def advanceFullTurn() = {
    val takenTurns = robots.take(turn)
    val toTakeTurn = robots.drop(turn)
    toTakeTurn.foreach(_.takeTurn())
    takenTurns.foreach(_.takeTurn())
  }
