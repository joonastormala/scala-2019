...
class RobotBody(val world: RobotWorld, initialLocation: GridPos, initialFacing: CompassDir) {
  ...
  def neighboringSquare(direction: CompassDir): Square = world.apply(this.location.neighbor(direction))
  ...
  def canMoveTowards(direction: CompassDir): Boolean = neighboringSquare(direction) != Wall
  ...
  /** Determines whether the robot is stuck or not, that is, if there are any squares around
    ...
    * for this purpose.
    * @see [[canMoveTowards]] */
  def isStuck = this.world.neighbors(this.location, false).count(_ == Wall) == 4
