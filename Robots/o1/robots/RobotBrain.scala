...
abstract class RobotBrain(initialName: String, val body: RobotBody) {
  ...
  /** Returns the coordinates that point at the square that is immediately in front
    * of this robot. */
  def locationInFront: GridPos = this.location.neighbor(facing)


  /** Returns the square that is immediately in front of this robot. */
  def squareInFront: Square = this.body.neighboringSquare(facing)
  ...
  def robotInFront: Option[RobotBrain] = squareInFront.robot.flatMap(_.brain)
  ...
  def moveCarefully() = {
    if (squareInFront.isEmpty && !squareInFront.isUnpassable) {
