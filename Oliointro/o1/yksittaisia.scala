...
package object o1 extends o1.util.ShortcutAliases {
  ...
  object tili {
    ...
    def talleta(talletus: Int) = saldo = this.saldo + max(talletus,0)
    def nosta(nosto: Int) = {
      var overdraw = min(max(nosto,0),this.saldo)
      this.saldo = this.saldo - overdraw
      overdraw
...
/*  class Hirvio(val tyyppi: String, val taysiKunto: Int) {
    ...
    def kuvaus = tyyppi + " " + hp + "/" + taysiKunto    
    def nykykunto = hp
    def vahingoitu(damage: Int) = (hp = hp - damage)
  ...
  def suomenLippu(leveys: Double) = {
    var a = rectangle(leveys, (leveys * 110 / 180), White)
    var b = rectangle(leveys, (leveys * 30 / 180), Blue)    
    var d = rectangle(leveys / 6, (leveys * 110 / 180), Blue)
    var e = rectangle((leveys * 50 / 180), (leveys * 110 / 180), White)
    var c = d.leftOf(e)
    var f = b.onto(c)
    var g = f.onto(a)
    g
