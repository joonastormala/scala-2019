...
class CarMap(tileSource: TileSource, center: Pos, zoomLevel: Int) extends MapPanel(tileSource) {
    ...
    contents += new MenuItem(Action("Fuel...")  {
      for {
        ...
        if (gainedFuel == 0.0 && amountWanted > 0.0) {
          ...
          display("Could only add " + gainedFuel + " liters.", RelativeTo(thisMap))
        }
      }
    })
  ...
  private def carAt(pixel: Point) = {
    ...
    if (this.carThreads.isEmpty) {
      None
    } else {
      val closest = this.carThreads.reverse.minBy(distance)
      if (distance(closest) == Double.PositiveInfinity) None else Some(closest)
    }
  ...
  def newRouteHighlight(route: IndexedSeq[Segment]): Unit = {
    ...
    def dropEvenlyTillMax(elements: IndexedSeq[Segment]) = {
      val dropRatio = (elements.size.toDouble / MaxNumberOfMarkers).max(1)
...
private object CarMap {
  ...
  private[carsim] def toDirectionsFormat(pos: Pos) = pos.x + "," + pos.y
