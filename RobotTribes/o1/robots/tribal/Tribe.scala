...
/** A "tribe" of robots is essentially a program which makes any robots who follow that program 
  ...
  * 
  *  - ''friendsdir N'': As ''enemiesdir'' above but counts friendly robots instead. Since this
  *              count will always include the acting robot itself, the result is between 1 and 5 
  *              (inclusive).
  * 
  *
  * ==Hacking and Talking==
...
class Tribe(val name: String, fileName: String) {
  ...
  def instructionAt(lineNumber: Int) = instructions.lift(lineNumber - 1).getOrElse(new TerminateProgram(lineNumber))
  ...
  object RoboSpeakGrammar extends JavaTokenParsers {
    ...
    def  enemiesdir = "enemiesdir"~>param            ^^ { case relDir => new EnemiesDir(relDir) } 
    def  friendsdir = "friendsdir"~>param            ^^ { case relDir => new FriendsDir(relDir) }
    def       shout = "shout"~>param                 ^^ { case line => new Shout(line) } 
    def        add1 = "add1"~>slotParam              ^^ { case slot => new Add1(slot.left.get) }
    def        plus = "plus"~>slotParam~param~param  ^^ { case slot1~value1~value2 => new Plus(slot1.left.get, value1, value2) }
    def       minus = "minus"~>slotParam~param~param ^^ { case slot1~value1~value2 => new Minus(slot1.left.get, value1, value2) }    
  ...
  abstract class ArithmeticInstruction(resultSlot: String, val operand1: Param, val operand2: Param, val compute: (Int, Int) => Int) extends LoadInstruction(resultSlot) {
    def newSlotValue(actor: TribalBot) = this.compute(this.eval(actor, this.operand1), this.eval(actor, this.operand2))
  }
  
  class Plus (resultSlot: String, operand1: Param, operand2: Param) extends ArithmeticInstruction(resultSlot, operand1, operand2, _ + _ )
  class Minus(resultSlot: String, operand1: Param, operand2: Param) extends ArithmeticInstruction(resultSlot, operand1, operand2, _ - _ )
  ...
  class UTurn extends SimpleInstruction( robo => { robo.spin(); robo.spin() } ) with ToNextLine with EndsTurn
  ...
  class Shout(line: Param) extends GenericInstruction with ToNextLine with ContinuesTurn {
	  def act(actor: TribalBot) = {
  ...
  class Add1(targetSlot: String) extends LoadInstruction(targetSlot) {
    def newSlotValue(actor: TribalBot) = this.slotValue(actor) + 1
  }

  
  abstract class RadarCount(val includeInCount: (TribalBot, RobotBrain) => Boolean) extends LoadInstruction(Tribe.RadarSlot) {
