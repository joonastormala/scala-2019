...
class TribalBot(body: RobotBody, val tribe: Tribe) extends RobotBrain(tribe.newName(), body) {
  ...
  def moveBody() = {
    var turnSpent = false
    if(this.robotInFront.isDefined){
      if(!isFriend(robotInFront.get) && !this.tribe.isPacifist) {
        this.hack()
        turnSpent = true
      }
    ...
    def playTurn(input: Instruction): Unit = {
      
      if(turnSpent){
        Unit
      }else if(input.endsTurn){
        nextInstruction = input.execute(this)
        turnSpent = true
      } else {
        playTurn(input.execute(this))
      }
    }
    playTurn(nextInstruction)
  }
  ...
  def determineTribe(another: RobotBrain): Option[Tribe] = {
    another match {
      case a: TribalBot => Some(a.tribe)
      case b: RobotBrain => None
    }
  }
  ...
  def isFriend(robot: RobotBrain) = {    
    determineTribe(robot) match {
      case Some(thing) => thing == this.tribe && robot != this.body.brain.get
      case None => false
    }
  }
  ...
  def talk(lineNumber: Int) = {
    if(seesFriend) {
      friendInFront.get.boot(lineNumber)
    }
  }
  ...
  /** "Brainwashes" an enemy robot that is currently right in front of the acting robot, 
    ...
    * @see [[enemyInFront]] */
  def hack() = {
    if(seesEnemy && !this.tribe.isPacifist){
      val robotsbody = enemyInFront.get.body
      val brain = (new TribalBot(robotsbody, tribe))
      brain.boot(memorySlots("hackline"))
      robotsbody.brain = Some(brain)
    }
  }
  ...
  def returnFromSubprogram(): Int = {
    ...
    this.callStack.pop.map(_.returnLine).getOrElse(failWithUnderflowError)
  }


  /** Returns the brains of all the robots in the same robot world with this one. */
  def longRadar = this.body.world.robotList.map(_.brain).filter(_.isDefined).map(_.get)
