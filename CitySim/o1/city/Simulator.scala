...
class Simulator {
  ...
  def findDemographic(demographic: Demographic): Vector[GridPos] = allAddresses.filter(cityMap.matches(_,demographic) == true)
  ...
  def dissatisfiedResidents: Vector[GridPos] = {
    def satisfied(location: GridPos) = {      
      allNeighbors(location).filter(_ == cityMap.elementAt(location)).size / allNeighbors(location).size.toDouble * 100 >= similarityDesired
    }
    def allNeighbors(location: GridPos) = {
      cityMap.neighbors(location, true).filter(_ != Vacant) //vektorin josta on poistettu vapaat, koko
    }
    def allVacant(location: GridPos): Boolean = {
      cityMap.neighbors(location, true).forall(_ == Vacant)  //returns true if all the neighbors are vacant
    }    
    
    def isVacant(location: GridPos): Boolean = {
      cityMap.elementAt(location) == Vacant //onko kyseisessä kohteessa oleva vapaa
    }
    allAddresses.filterNot(satisfied(_)).filterNot(allVacant(_)).filterNot(isVacant(_))
  }
  ...
  def moveResidents() = {
    ...
    val dissatisfiedAdresses = dissatisfiedResidents.toBuffer
    if (vacantAddresses.nonEmpty) {
      for (i <- Random.shuffle(dissatisfiedAdresses)) {
        var randomFree = Random.shuffle(vacantAddresses)
        var newAddress = randomFree(0)
        cityMap.swap(i, newAddress)
        vacantAddresses -= newAddress
        vacantAddresses += i              
      }
    }
  }
